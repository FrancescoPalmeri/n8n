...
export class PlaySignage implements INodeType {
	description: INodeTypeDescription = {
				...
				},
        {
					...
					value: 'get-playlists',
					description: 'Get a list of all available playlists',
				},
				{
					name: 'Create Tag',
					value: 'create-tag',
				...
				},
        {
					name: 'Update Tag',
					value: 'update-tag',
					description: 'Update a Tag setting the playlist',
				},
				{
					name: 'Activate Tag',
					value: 'activate-tag',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			switch (operations) {
				...
				case 'get-tags':
					...
					break;
        case 'get-playlists':
          responseData = await playSignageApiRequest.call(this, 'GET', 'playlists', {});
          break;
				case 'remove-tag':
        ...
        case 'update-tag':
          ...
          const playlist = this.getNodeParameter('playlist', i) as string;
          const screens = this.getNodeParameter('screens', i) as string;
          responseData = await playSignageApiRequest.call(this, 'PUT', `tags/${tagUUID}`, {name: name, playlist: {id: playlist}, screens: [{id: screens}]});
          break;
				default:
					responseData = [];
					break;
